...
export class AppService {
  ...
  async getHD(dto: DownloadVideo): Promise<{ fileLocation: string }> {
    ...
    try {
      if (hasAudio === true) {
        ...
        const downloadVideo = this.downloadVideo({
          itag,
          outputLocation: videoOutput,
          videolink: videoUrl,
        });

        const downloadAudio = this.downloadAudio({
  ...
  async sendAvailableQualities(dto: UrlValidation) {
    try {
      ...
      // function filterAndSortUniqueQualityLabel(metadata) {
      ...
      //       !uniqueQualityLabels.has(item.qualityLabel)
      //     ) {
      ...
      //   // Sort the filteredData array based on qualityLabel

      //   return filteredData;
      // }

      // const qualityLabel = filterAndSortUniqueQualityLabel(videoInfo.formats);
